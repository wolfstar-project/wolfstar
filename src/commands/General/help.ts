...
export class UserCommand extends WolfCommand {
	public override async messageRun(message: Message, args: WolfCommand.Args, context: WolfCommand.RunContext) {
		if (args.finished) {
	...
	private getCommandPrefix(context: WolfCommand.RunContext): string {
	...
	private async helpCategories(message: Message, args: WolfCommand.Args) {
	...
	private async all(message: Message, args: WolfCommand.Args, context: WolfCommand.RunContext) {
	...
	private async display(message: Message, args: WolfCommand.Args, index: number | null, context: WolfCommand.RunContext) {
	...
	@RequiresClientPermissions(PermissionFlagsBits.EmbedLinks)
	private async buildCommandHelp(message: Message, args: WolfCommand.Args, command: WolfCommand, prefixUsed: string) {
	...
	private formatCommand(t: TFunction, prefix: string, paginatedMessage: boolean, command: WolfCommand) {
