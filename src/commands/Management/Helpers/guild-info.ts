...
export class UserCommand extends WolfCommand {
	...
	private async buildDisplay(args: WolfCommand.Args, roles: Role[], color: number): Promise<WolfPaginatedMessage> {
	...
	private async getSummary(args: WolfCommand.Args, roles: Role[], color: number): Promise<EmbedBuilder> {
		const guild = args.message.guild!;
	...
	private getBanner(args: WolfCommand.Args, color: number): EmbedBuilder {
		const guild = args.message.guild!;
	...
	private getIcon(args: WolfCommand.Args, color: number): EmbedBuilder {
		const guild = args.message.guild!;
		return this.getImage(args.t(LanguageKeys.Commands.Management.GuildInfoIcon), guild.iconURL({ size: 4096, extension: 'png' })!, color);
	}

	private getSplash(args: WolfCommand.Args, color: number): EmbedBuilder {
		const guild = args.message.guild!;
	...
	private getDiscoverySplash(args: WolfCommand.Args, color: number): EmbedBuilder {
		const guild = args.message.guild!;
