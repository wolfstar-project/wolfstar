...
export class UserCommand extends WolfCommand {
	...
	public override async messageRun(message: GuildMessage, args: WolfCommand.Args, context: WolfCommand.RunContext) {
		...
		if (await promptConfirmation(message, t(LanguageKeys.Commands.Moderation.ActionSharedRoleSetupExisting))) {
			...
			if (result.isOk()) {
				...
				await writeSettings(message.guild, { rolesMuted: role.id }, message.author.id);
	...
	private async askForRole(message: GuildMessage, args: WolfCommand.Args, context: WolfCommand.RunContext): Promise<Result<Role, UserError>> {
