import { PermissionNodeAction, readSettings, readSettingsPermissionNodes, writeSettingsTransaction, type PermissionsNode } from '#lib/database';
...
export class UserCommand extends WolfSubcommand {
	public async add(message: GuildMessage, args: WolfSubcommand.Args) {
		...
		await trx.write({ [propertyKey]: nodes.add(target, command, action) } as any).submitWithAudit(message.author.id);
	...
	public async remove(message: GuildMessage, args: WolfSubcommand.Args) {
		...
		const nodes = readSettingsPermissionNodes(trx.settings);
		const propertyKey = nodes.settingsPropertyFor(target);
		await trx.write({ [propertyKey]: nodes.remove(target, command, action) } as any).submitWithAudit(message.author.id);
	...
	public async reset(message: GuildMessage, args: WolfSubcommand.Args) {
		...
		const nodes = readSettingsPermissionNodes(trx.settings);
		const propertyKey = nodes.settingsPropertyFor(target);
		await trx.write({ [propertyKey]: nodes.reset(target) } as any).submitWithAudit(message.author.id);
	...
	private async showOne(message: GuildMessage, args: WolfSubcommand.Args) {
		const target = await args.pick('roleName').catch(() => args.pick('member'));
	...
	private async formatPermissionNodes(args: WolfSubcommand.Args, nodes: readonly PermissionsNode[], isRole: boolean) {
		const output: string[] = [];
	...
	private formatPermissionNode(args: WolfSubcommand.Args, node: PermissionsNode, isRole: boolean, target: Role | GuildMember) {
		return [
	...
	private formatCommands(args: WolfSubcommand.Args, commands: readonly string[]) {
