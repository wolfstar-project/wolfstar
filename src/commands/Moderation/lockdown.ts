...
import { LockdownManager, WolfSubcommand } from '#lib/structures';
...
export class UserCommand extends WolfSubcommand {
	public override messageRun(message: GuildMessage, args: WolfSubcommand.Args, context: WolfSubcommand.RunContext) {
	...
	public async auto(message: GuildMessage, args: WolfSubcommand.Args) {
	...
	public async unlock(message: GuildMessage, args: WolfSubcommand.Args) {
		const role = await args.pick('roleName').catch(() => message.guild.roles.everyone);
		const channel = args.finished ? assertNonThread(message.channel) : await args.pick('textChannelName');
		return this.handleUnlock(message, args, role, channel);
	}

	public async lock(message: GuildMessage, args: WolfSubcommand.Args) {
		const role = await args.pick('roleName').catch(() => message.guild.roles.everyone);
		const channel = args.finished ? assertNonThread(message.channel) : await args.pick('textChannelName');
		const duration = args.finished ? null : await args.pick('timespan', { minimum: 0 });
		return this.handleLock(message, args, role, channel, duration);
	...
	private async handleUnlock(message: GuildMessage, args: WolfSubcommand.Args, role: Role, channel: NonThreadGuildTextBasedChannelTypes) {
