...
export class UserCommand extends WolfSubcommand {
	...
	private async handlePurge(message: GuildMessage, args: WolfSubcommand.Args, filter: BooleanFn<[GuildMessage]>, silent = false) {
		...
		const content = args.t(LanguageKeys.Commands.Moderation.PruneAlert, { count: messages.size, total: limit });
	...
	private async purgeBulk(
	...
	): Promise<Collection<string, GuildMessage | PartialMessage>> {
		...
		const msgDeleted = new Collection<string, GuildMessage | PartialMessage>();
		...
		const BulkDelete = async (messageIds: string[]) => {
			for (const chunked of chunk(messageIds, 100)) {
				let msgs = new Collection<Snowflake, Message | PartialMessage | undefined>();
				try {
					msgs = await (message.channel as TextChannel).bulkDelete(chunked);
				} catch (error) {
					...
					if (!hasError)
						msgs.filter((msg): msg is GuildMessage => !isNullOrUndefined(msg) && isGuildMessage(msg)).reduce(
							(col, message) => col.set(message.id, message),
							msgDeleted
						);
				}
			}
		...
		const fetchMessages = async (before?: string, after?: string) => {
			...
			this.resolveKeys([...filtered.keys()], position, limit);
