...
export class UserCommand extends WolfCommand {
	...
	private async fetchContext() {
		if (!this.#cachedEvalContext) {
			this.#cachedEvalContext = {
				...
				timers: { ...(await import('node:timers')), promises: await import('node:timers/promises') } as typeof import('node:timers') & {
					promises: typeof import('node:timers/promises');
				},
				discord: {
	...
	private async eval(message: Message, args: WolfCommand.Args, code: string, timeout: number): Promise<EvalResult> {
