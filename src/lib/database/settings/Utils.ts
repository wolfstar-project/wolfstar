...
export async function set(settings: Readonly<GuildEntity>, key: SchemaKey, args: WolfArgs): Promise<Partial<GuildData>> {
...
export async function remove(settings: Readonly<GuildEntity>, key: SchemaKey, args: WolfArgs): Promise<Partial<GuildData>> {
	const parsed = await key.parse(settings, args);
	if (key.array) {
		const { serializer } = key;
		const values = settings[key.property] as any[];
