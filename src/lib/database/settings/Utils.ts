...
export async function set(settings: GuildEntity, key: SchemaKey, args: WolfArgs) {
...
export async function remove(settings: GuildEntity, key: SchemaKey, args: WolfArgs) {
	const parsed = await key.parse(settings, args);
	if (key.array) {
		const values = Reflect.get(settings, key.property) as any[];
		const { serializer } = key;
