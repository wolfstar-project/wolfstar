 ...
 *
 * Wolf's actions will always call the `set` method before an action is taken,
...
export abstract class LoggerTypeManager {
	 ...
	 * when an action was taken by Wolf.
	...
	public async wait(id: Snowflake, signal?: AbortSignal): Promise<LoggerTypeContext | null> {
		// 1. If the context data was set beforehand by Wolf, return it
		// immediately.
		...
		// 2. The action was not taken by Wolf, fallback to audit logs.
