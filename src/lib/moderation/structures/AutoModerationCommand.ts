...
import { MessageFlags, PermissionFlagsBits, chatInputApplicationCommandMention, strikethrough, type Guild } from 'discord.js';

const Root = LanguageKeys.Commands.AutoModeration;
const RootModeration = LanguageKeys.Moderation;

export abstract class AutoModerationCommand extends WolfSubcommand {
	...
	/** @deprecated */
	public messageRunShow(message: GuildMessage, args: WolfSubcommand.Args) {
	...
	/** @deprecated */
	public messageRunEdit(message: GuildMessage, args: WolfSubcommand.Args) {
		const command = chatInputApplicationCommandMention(this.name, 'edit', this.getGlobalCommandId());
		return send(message, args.t(LanguageKeys.Commands.Shared.DeprecatedMessage, { command }));
	}

	/** @deprecated */
	public messageRunReset(message: GuildMessage, args: WolfSubcommand.Args) {
...
export namespace AutoModerationCommand {
	export type LoaderContext = WolfSubcommand.LoaderContext;

	/**
	 * The AutoModerationCommand Options
	 */
	export interface Options extends Omit<WolfSubcommand.Options, 'detailedDescription'> {
	...
	export type Interaction = WolfSubcommand.Interaction;
}
