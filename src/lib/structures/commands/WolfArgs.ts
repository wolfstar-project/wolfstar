...
export class WolfArgs extends Args {
	 ...
	 */
	public nextSplitResult({ delimiter = ',', times = Infinity }: WolfArgs.NextSplitOptions = {}): Result<string[], UserError> {
	 ...
	 * @returns An array of values.
	 */
	public nextSplit(options?: WolfArgs.NextSplitOptions) {
	...
	public static from(command: MessageCommand, message: Message, parameters: string, context: MessageCommand.RunContext, t: TFunction) {
		...
		return new WolfArgs(message, command, args, context, t);
	}
}

export namespace WolfArgs {
